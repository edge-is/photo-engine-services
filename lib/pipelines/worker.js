...
var createPipeline = function (globalConfig, namespace, log, client) {
  ...
  return {
    fetchFile : function (job, callback){
      bumbProgress(job);
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path }, `Starting fetching file`);
      var start = epoch();
      ...
      if (job.data.file.location === 'filesystem'){
        return fs.readFile(job.data.file.path, function (err, buffer, x){
          ...
          log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time : diff },`Finshed fetching file`);
    ...
    readExif : function (job, callback){
      bumbProgress(job);
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path },`Starting reading exif`);
      ...
      exif.read(job.data.sourceImage.buffer, function (err, res){
        ...
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff },`Done reading exif`);
    ...
    createWatermark : function (job, callback){
      ...
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path },`Starting create watermark`);
      ...
      selectWatermarkOwerlayImage(job.data.sourceImage.metadata, function (err, watermarkBuffer, metadata){
        ...
        sharp(job.data.sourceImage.buffer)
          ...
          .toBuffer(function (err, buffer){
            ...
            log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff },`Done creating watermark`);
    ...
    createSource : function (job, callback){
      bumbProgress(job);
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path },`Starting create source watermark`);
      ...
      sharp(job.data.originalImage.buffer)
        ...
        .toBuffer(function (err, buffer ,metadata){
          ...
          log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff },`Done create source image`);
    ...
    checkThumbnailsExists : function (job, callback){
      ...
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path },`Checking if thumbnails already exist`);
      ...
      async.eachOfLimit(job.data.thumbnails, 4, function (value, key, next){
        ...
        u.exists(location, function (err, stats){
          ...
          if (err){
            if (err.code !== 'ENOENT') log.warn({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Error checking file for exists` , err);
          }
          var e = false;
          if (stats){
            e = true;
          ...
          log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Checking:${location} exists: ${e}`);
          next();
        });
      }, function done(){
        var diff = ( epoch() - start );
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff },`Done checking if thmbnails exists`);

        if (thumnailsThatExist.length > 0) {

          log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Thumbnails exist`)
    ...
    createThumbnails : function (job, callback){
      ...
      if(job.data.thumbnailsExists && !_config.thumbnails.force)  {
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Thumbnails already exist skipping`);
        return callback(null, job);
      }

      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Starting create thumbnails for`);
      ...
      async.eachOfLimit(job.data.thumbnails, 1, function (value, key, next){
        ...
        sharp(inputBuffer)
          ...
          .toBuffer(function (err, buffer, metadata){
            ...
            log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff }, `Done creating thumbnail: ${key}`);
        ...
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff }, `Done create thumbnails`);
    ...
    indexExif : function (job, callback){
      ...
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Starting indexing`);
      ...
      client.index(body, function indexElasticsearch(err, response){
        if (err) {
          ...
          if (!_config.index.faildir) {
            log.warning({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `no dump directory defined`)
            return callback(err);
          }
          log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path}, `Dumping metadata into faildir: ${_config.index.faildir}`);
        ...
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff }, `Done indexing`);
    ...
    storeThumbnails : function (job, callback){
      ...
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path },`Starting storing thumbnails`);
      // store thumbnails..
      //
      ...
      async.eachOfLimit(job.data.thumbnails, 1, function (value, key, next){
        ...
        log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff }, `Done storing thumbnails`);
    ...
    storeSource : function (job, callback){
      ...
      log.debug({ jobID: job.id, fileID :job.data.fileID , file:job.data.path }, `Starting storing source images`);
      ...
      u.createDirecotry(dir, function (err){
        ...
        fs.writeFile(destination, job.data.sourceImage.buffer, function (err, res){
          ...
          log.info({ jobID: job.id, fileID :job.data.fileID , file:job.data.path, time:diff }, `Done storing source images`);

...
function cleanXML(xml){
  ...
  const start = new RegExp('<x:xmpmeta');
  const stop = new RegExp('</x:xmpmeta');
  const lines = xml.split('\n');

  let capturing = false;

  const arr = lines.filter(function (line, index, set){
    const lastLine = set[index - 1]
    if (start.test(line)){
      capturing = true;
    }
    if (stop.test(lastLine)){
      capturing = false;
    }
    return capturing;
  });
...
async function captureXMP(content){
  const start = content.indexOf('<x:xmpmeta');
  const end = content.lastIndexOf('</x:xmpmeta>')

  let xml = content.substring(start, end + '</x:xmpmeta>'.length);
  xml = cleanXML(xml);
  xml = ['<xmlcontent>', xml, '</xmlcontent>'].join('')
...
XMP.prototype.read = async function (fileOrBuffer) {
  ...
  if (!isBuffer){
    ...
    arrayOfMetadata = arrayOfMetadata.map(function (item){
      ...
      // Add archive -- DEPRICATED

      if (!item.archive){
        item.archive = item.Directory.split(path.sep).pop();
      }

      if (!item.Filename){
        item.Filename = parsed.base;
        item.FileName = parsed.name;
      }

      if (!item.SourceFile){
        item.SourceFile = parsed.base;
      }
  ...
  arrayOfMetadata = cleanStrings(arrayOfMetadata)

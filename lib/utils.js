...
function createDirecotry(directory, callback){


  fs.access(directory, fs.W_OK, function (err, res){
...
function service(time, callback){
  ...
  var int = setInterval(callback, seconds);
...
function getThumbnailsName(filename, config){


  var nameObjectHash = createFilenameObject(filename);


  nameObjectHash.format = config.format;
  var output = {};

  for (var profileName in config.profile){
    var profile = config.profile[profileName];
    nameObjectHash.profile = profileName;
    output[profileName] = {
      name : formatString(config.name, nameObjectHash),
      buffer : null,
      profile : profile
    };
...
module.exports = {
  ...
  slug : _slug,
  md5 : md5,
  getThumbnailsName : getThumbnailsName,
  createDirecotry : createDirecotry
}
